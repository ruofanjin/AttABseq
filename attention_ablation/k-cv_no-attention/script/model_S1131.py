...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x = x.view(bsz, -1, self.n_heads * (self.hid_dim*3 // self.n_heads))
...
class Predictor(nn.Module):
    ...
    def forward(self, ag_s, ab_s, ag_m_s, ab_m_s, ag_s_num, ab_s_num, ag_m_s_num, ab_m_s_num, itera, correct_interaction, fold):
        ...
        if os.path.exists('../1131analysis/fold{}/ag{}.txt'.format(fold, itera)):
            f1 = open('../1131analysis/fold{}/ag{}.txt'.format(fold, itera),'a+')
            f1.write(str(correct_interaction))
            f1.write('\n')
            f1.write(str(ag_ab_norm.tolist()))
            f1.write('\n')
        else:
            f1 = open('../1131analysis/fold{}/ag{}.txt'.format(fold, itera),'w')
            f1.write(str(correct_interaction))
            f1.write('\n')
            f1.write(str(ag_ab_norm.tolist()))
            f1.write('\n')
        
        if os.path.exists('../1131analysis/fold{}/ab{}.txt'.format(fold, itera)):
            f2 = open('../1131analysis/fold{}/ab{}.txt'.format(fold, itera),'a+')
            f2.write(str(correct_interaction))
            f2.write('\n')
            f2.write(str(ab_ag_norm.tolist()))
            f2.write('\n')
        else:
            f2 = open('../1131analysis/fold{}/ab{}.txt'.format(fold, itera),'w')
            f2.write(str(correct_interaction))
            f2.write('\n')
            f2.write(str(ab_ag_norm.tolist()))
            f2.write('\n')
        
        if os.path.exists('../1131analysis/fold{}/ag_mut{}.txt'.format(fold, itera)):
            f3 = open('../1131analysis/fold{}/ag_mut{}.txt'.format(fold, itera),'a+')
            f3.write(str(correct_interaction))
            f3.write('\n')
            f3.write(str(ag_ab_m_norm.tolist()))
            f3.write('\n')
        else:
            f3 = open('../1131analysis/fold{}/ag_mut{}.txt'.format(fold, itera),'w')
            f3.write(str(correct_interaction))
            f3.write('\n')
            f3.write(str(ag_ab_m_norm.tolist()))
            f3.write('\n')
        
        if os.path.exists('../1131analysis/fold{}/ab_mut{}.txt'.format(fold, itera)):
            f4 = open('../1131analysis/fold{}/ab_mut{}.txt'.format(fold, itera),'a+')
            f4.write(str(correct_interaction))
            f4.write('\n')
            f4.write(str(ab_ag_m_norm.tolist()))
            f4.write('\n')
        else:
            f4 = open('../1131analysis/fold{}/ab_mut{}.txt'.format(fold, itera),'w')
            f4.write(str(correct_interaction))
            f4.write('\n')
            f4.write(str(ab_ag_m_norm.tolist()))
            f4.write('\n')
