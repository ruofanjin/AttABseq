...
class feature(object):
    ...
    def seq2pssm(self):
        with open('645aa.fasta', 'w') as f:
            f.write('>name\n')
            f.write(self.seq)
        f.close()
        os.system('../ncbi-blast-2.12.0+/bin/psiblast -query 645aa.fasta -db ../ncbi-blast-2.12.0+/bin/swissprot -num_iterations 3 -out 645.txt -out_ascii_pssm 645aa.pssm')
        with open('645aa.pssm', 'r') as inputpssm:
            count = 0
            pssm_matrix = []
...
if __name__ == "__main__":
    ...
    csv = pd.read_csv('../data/AB645.csv',usecols=['PDB','Mutation','antibody_light_seq','antibody_heavy_seq','antigen_a_seq','antigen_b_seq','antibody_light_seq_mut','antibody_heavy_seq_mut','antigen_a_seq_mut','antigen_b_seq_mut', 'ddG'])
    ...
    train_index, val_index = split('../data/AB645.csv', scale=0.1)
    ...
    with open('../645analysis/split.txt','w') as file:
    ...
    """Output files."""
    file_loss_min_PCCS = '../output645/loss_min_result/RECORD.txt'
    file_loss_min_model = '../output645/loss_min_model/model'
    file_best_pcc_PCCS = '../output645/best_pcc_result/RECORD.txt'
    file_best_pcc_model = '../output645/best_pcc_model/model'
    file_best_r2_PCCS = '../output645/best_r2_result/RECORD.txt'
    file_best_r2_model = '../output645/best_r2_model/model'
