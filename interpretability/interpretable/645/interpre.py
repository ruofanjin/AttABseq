...
dir = '/home/ruofan/bcr/split/interpretable/645'
pdbidx = idx(dir+'/split.txt')
ls_pdb, ls_mut, ls_ab,ls_ag,ls_abm,ls_agm, ls_label = seqs(pdbidx,dir+'/AB645.csv')
count = len(ls_label)
ab = open('ab16.txt').readlines()[-2*count:]
ag = open('ag16.txt').readlines()[-2*count:]
abm = open('ab_mut16.txt').readlines()[-2*count:]
agm = open('ag_mut16.txt').readlines()[-2*count:]
ab_inters = []
ag_inters = []
abm_inters = []
agm_inters = []
...
for idx in range(count):
    ...
    heatmap_make(ab_ag_csv, dir+'/interpre_heatmap'+'/ab-{}-{}-{}-{}.png'.format(name, stru_mutation, seq_mutation, idx))
